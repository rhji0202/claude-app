...
**M3. 진행 관측 부재** ✅ 해소(4단계)
- (기존) 이슈 실행은 시작/종료 경계만 기록, 실행 중 tool/text 이벤트를 볼 수 없었다.
- **해소:** 이슈 실행을 `runStream` 기반(`IssuesService.runViaStream`)으로 전환해 현재 도구/활동을 `IssueTask.progress`에 throttle(2초) 저장하고, 이미 있는 4초 폴링으로 목록에서 "도구: Bash" 등을 표시. 별도 SSE 엔드포인트 없이 관측 확보.
- 추가로 **알림**(NotifyService): 프로젝트별 webhook으로 크론 실패·이슈 완료/실패·PR 생성 이벤트 발신.
